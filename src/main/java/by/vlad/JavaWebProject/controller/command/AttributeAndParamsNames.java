package by.vlad.JavaWebProject.controller.command;

public class AttributeAndParamsNames {
    private AttributeAndParamsNames(){
    }

    //session attributes names
    public static final String USER_LOGIN = "user_login";
    public static final String USER_NAME = "user_name";
    public static final String USER_SURNAME = "user_surname";
    public static final String USER_EMAIL = "user_email";
    public static final String USER_ROLE = "user_role";
    public static final String USER_PHONE_NUMBER = "user_phone_number";
    public static final String USER_SERIAL_NUMBER = "user_serial_number";
    public static final String CURRENT_PAGE = "current_page";

    //request params names
    public static final String BOOK_ID = "book_id";
    public static final String EMAIL = "email";
    public static final String PASSWORD = "pass";

    public static final String NEW_LOGIN = "new_login";
    public static final String NEW_NAME = "new_name";
    public static final String NEW_SURNAME = "new_surname";
    public static final String NEW_EMAIL = "new_email";
    public static final String NEW_PHONE_NUMBER = "new_phone_number";
    public static final String NEW_SERIAL_NUMBER = "new_serial_number";
    public static final String NEW_PASSWORD = "new_password";
    public static final String REPEATED_PASSWORD = "repeated_pass";

    public static final String UPDATE_LOGIN = "update_login";
    public static final String UPDATE_NAME = "update_name";
    public static final String UPDATE_SURNAME = "update_surname";
    public static final String UPDATE_EMAIL = "update_email";
    public static final String UPDATE_PHONE_NUMBER = "update_phone_number";
    public static final String UPDATE_SERIAL_NUMBER = "update_serial_number";
    public static final String NEW_PASS = "new_pass";
    public static final String REPEAT_NEW_PASS = "repeat_new_pass";

    //request attributes names
    public static final String AUTHORS = "authors";
    public static final String BOOK = "book";
    public static final String BOOKS_LIST = "books_list";
    public static final String IMAGES = "images";

}
